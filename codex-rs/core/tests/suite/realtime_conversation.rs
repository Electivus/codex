...
const WEBSOCKET_HANDSHAKE_TIMEOUT: Duration = Duration::from_secs(/*secs*/ 10);
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    assert!(
        server
            .wait_for_handshakes(/*expected*/ 1, WEBSOCKET_HANDSHAKE_TIMEOUT)
            .await
    );
...
async fn conversation_webrtc_start_posts_generated_session() -> Result<()> {
    ...
    let expected_session: Value =
        serde_json::from_str(session).context("expected session part should be valid json")?;
